...
- **Deleted file UX (Issue #5):** `TreeNode` uses `[DataMember]` + `INotifyPropertyChanged` for every bindable property. New boolean flags (like `IsDeleted`) need both attributes and full property-change notification to work correctly with the VS extensibility XAML engine. The `ChangeKind` string already existed on the node, but a dedicated boolean is cleaner for XAML `DataTrigger` binding. `DataTrigger` on `TextBlock` is the right way to apply `TextDecorations="Strikethrough"` conditionally — a global `Style` would clobber the VS-themed foreground, so the trigger must also carry a `Foreground` setter in a local style. For deleted files in `OpenDiffCommand`, the right-side path (`fileInfo.FullPath`) no longer exists on disk; the fix is to write a temp empty file and mark it with `VSDIFFOPT_RightFileIsTemporary` so VS doesn't try to open the missing path.
