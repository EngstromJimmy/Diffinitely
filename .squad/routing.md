...
|-----------|----------|----------|
| Architecture, design decisions, scope | Bruce 🏗️ | API design, structural trade-offs, what to build next |
| Code review | Bruce 🏗️ | Review all agent work, flag structural issues |
| WPF/XAML, UI layout, ViewModel, converters | Selina ⚛️ | ToolWindow UI, data binding, visual behavior |
| GitHub API, Git services, data models, commands | Lucius 🔧 | PR fetching, diff loading, extension commands |
| Tests, edge cases, quality gates | Renee 🧪 | Unit tests, mocking GitHub API, error scenario coverage |
| Authentication, credentials, token handling | Gordon 🔒 | GitHub OAuth, secure storage, auth flow review |
| Session logging, decision merging | Scribe 📋 | Automatic — never needs routing |
| Work queue, issue monitoring | Ralph 🔄 | Backlog scanning, PR status, automated triage |
